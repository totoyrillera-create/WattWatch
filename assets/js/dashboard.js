// WattWatch — dashboard client-side behavior
// Polls api/get_live_data.php every 10s to refresh stat cards + equipment
// power figures without a full page reload. Chart drawing (if a
// #powerChart canvas is present) is handled by pages/dashboard.php inline,
...
(function () {
  ...
  async function pollLiveData() {
    ...
    if (!root) return;
    try {
      const res = await fetch(window.WATTWATCH_BASE_URL + '/api/get_live_data.php', { credentials: 'same-origin' });
      if (!res.ok) return;
