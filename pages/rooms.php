...
requireLogin();
if (!can('manage_rooms') && !can('manage_equipment')) { http_response_code(403); require __DIR__.'/403.php'; exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add_room' && can('manage_rooms')) {
            ...
            logActivity($user['user_id'], 'add_room', 'Added room "' . $_POST['room_name'] . '"');
            $notice = 'Room added.';
        } elseif ($action === 'delete_room' && can('manage_rooms')) {
            $stmt = $pdo->prepare("DELETE FROM rooms WHERE room_id = ?");
            $stmt->execute([$_POST['room_id']]);
            logActivity($user['user_id'], 'delete_room', 'Deleted room #' . $_POST['room_id']);
            $notice = 'Room deleted.';
            ...
            logActivity($user['user_id'], 'add_equipment', 'Added equipment "' . $_POST['equipment_name'] . '"');
            ...
            logActivity($user['user_id'], 'update_equipment', 'Set equipment #' . $_POST['equipment_id'] . ' status to ' . $_POST['status']);
            ...
            logActivity($user['user_id'], 'delete_equipment', 'Deleted equipment #' . $_POST['equipment_id']);
...
$pageTitle = 'Rooms / Equipment';
$pageSubtitle = 'Manage monitored rooms and connected equipment';
