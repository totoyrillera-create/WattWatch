...
if (($_GET['export'] ?? '') === 'csv') {
    logActivity($user['user_id'], 'export_report', "Exported report $start to $end");
...
$pageTitle = 'Reports';
$pageSubtitle = 'Historical energy consumption summaries';
...
<div class="card-grid">
  <div class="stat-card">
    ...
    <div><p class="stat-label">Total Energy (Range)</p><p class="stat-value"><?= formatKwh($totalKwh) ?></p></div>
  </div>
  <div class="stat-card">
  ...
  <table>
    ...
    <?php foreach ($summary as $row): ?>
      <tr>
        ...
        <td><?= formatKwh((float)$row['total_kwh']) ?></td>
