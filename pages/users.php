...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add_user') {
            ...
            logActivity($me['user_id'], 'add_user', "Created user \"$username\"");
            $notice = 'User created.';
            ...
            logActivity($me['user_id'], 'update_user_role', "Changed role for user #$targetId");
            $notice = 'Role updated.';
            ...
            logActivity($me['user_id'], 'update_user_status', "Set user #$targetId to " . $_POST['status']);
            ...
            logActivity($me['user_id'], 'reset_user_password', "Reset password for user #$targetId");
...
$pageTitle = 'Users';
$pageSubtitle = 'Manage accounts and role-based privileges';
    ...
    <table>
      ...
      <?php foreach ($users as $u): $isMe = $u['user_id'] == $me['user_id']; ?>
        <tr>
          ...
          <td class="stat-sub"><?= $u['last_login'] ? timeAgo($u['last_login']) : 'never' ?></td>
